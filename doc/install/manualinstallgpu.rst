...
* **siteResources=/opt/shifter/site-resources** Absolute path to where host software dependencies (e.g. nvidia) will be bind-mounted into the container. This feature is used to provide the docker image with access to drivers and hardware that are unique to the target machine, this is done by bind mounting them into the path provided in siteResources when the container is started on the target machine.
